...
run() {
	...
	if [ "$source" = "reference" ]; then
		echo "lifting not needed"
	else
		if [ -z $cov_base ]; then echo "no covariance specified (skip)"; return 1; fi
		sequence_in_file=$(echo "$all_cons" | grep "MP$cov")
	fi

	# https://github.com/crukci-bioinformatics/docker-crossmap
	# CrossMap.py bed mm9ToMm10.over.chain.gz targets.mm9.bed targets.mm10.bed
	...
	if [ ! -z "$sequence_in_file" ]; then
		echo "todo: CrossMap.py bed /data/cons/${sequence_in_file/.fasta/.chain} /data/filter/$filter_file $lift_file"
		echo "CrossMap.py bed /data/cons/${sequence_in_file/.fasta/.chain} /data/filter/$filter_file $lift_file" >> ${fasta_dir}/_call.sh
	else
		echo "reference or consensus fasta file missing (skipping)"
	fi
}
...
$script_directory/../run_daemon.sh "$docker_switches" "" "$docker_volumes" "crossmap" "/output/_call.sh" "/dev/null"
if [ $? -ne 0 ]; then echo "container returns non-zero code"; exit 1; fi

# TODO make dedicated folder for liftover and coordinates data
